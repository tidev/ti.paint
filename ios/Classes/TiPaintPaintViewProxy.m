...
- (void)undo:(id)args {
  [[self view] performSelectorOnMainThread:@selector(undo:)
                                withObject:args
                             waitUntilDone:NO];
}

- (void)redo:(id)args {
  [[self view] performSelectorOnMainThread:@selector(redo:)
                                withObject:args
                             waitUntilDone:NO];
}
...
- (void)playbackDrawing:(id)args {
  [[self view] performSelectorOnMainThread:@selector(playbackDrawing:)
                                withObject:args
                             waitUntilDone:NO];
}

- (void)pausePlayback:(id)args {
  [[self view] performSelectorOnMainThread:@selector(pausePlayback:)
                                withObject:args
                             waitUntilDone:NO];
}

- (void)resumePlayback:(id)args {
  [[self view] performSelectorOnMainThread:@selector(resumePlayback:)
                                withObject:args
                             waitUntilDone:NO];
}

- (void)stopPlayback:(id)args {
  [[self view] performSelectorOnMainThread:@selector(stopPlayback:)
                                withObject:args
                             waitUntilDone:NO];
}

- (void)setPlaybackSpeed:(id)args {
  [[self view] performSelectorOnMainThread:@selector(setPlaybackSpeed:)
                                withObject:args
                             waitUntilDone:NO];
}
...
- (void)loadStrokes:(id)args {
  [[self view] performSelectorOnMainThread:@selector(loadStrokes:)
                                withObject:args
                             waitUntilDone:NO];
}
